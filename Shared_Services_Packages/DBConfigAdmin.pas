...
type
  TDBConfigAdminForm = class(TForm)
...
var
  DBConfigAdminForm: TDBConfigAdminForm;

implementation

{$REGION 'FORM'}

procedure TDBConfigAdminForm.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
var
  dlgRes : integer;
begin
...
procedure TDBConfigAdminForm.init(syn: boolean = false);
var
  files : TArray<string>;
  I : integer;
begin
  ...
  begin
    synergize := true;
    iniDir := synIniLoc;
    DBConfigAdminForm.Caption := 'Synergize Configuration';
...
procedure TDBConfigAdminForm.bSelectClick(Sender: TObject);
var
  config : TIniFile;
begin
  if (dbList.Items.Count <> 0) and (dbList.ItemIndex <> -1) and (dbList.Items[dblist.ItemIndex] <> '') then
  begin
    //set genini database to this value and close form
    try
      config := TiniFile.Create(GenIniLoc);   //change to user INI
    except
      on E: Exception do
      begin
